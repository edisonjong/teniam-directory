...
import { cn, getItemTargetLinkInWebsite } from '@/lib/utils';
import type { ItemInfo } from '@/types';
import { ArrowUpRightIcon, Globe } from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';
import { Button } from '../ui/button';
      ...
      <div className="relative  flex flex-col h-full">
        ...
        <div className="flex justify-between items-center gap-3 border-t border-dashed pt-6 border-border mt-auto">
          ...
          {/* <Button
            size="sm"
            className={cn(
              'overflow-hidden rounded-full bg-sky-500 hover:bg-sky-600 gap-1 pr-2',
              'group transition-transform duration-300 ease-in-out hover:scale-105'
            )}
          >
            <div className="flex items-center justify-center gap-2">
              <span className="text-white font-semibold">Visit Website</span>
              <ArrowUpRightIcon className="text-white font-semibold size-3.5 transition-transform group-hover:translate-x-1" />
            </div>
          </Button> */}
        </div>
      </div>
